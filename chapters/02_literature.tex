...
propaganda is targeted for a specific goal, framing instead is what is used to do propaganda. Framing exists also without propaganda, as for example
...
We can take from the literature the following techniques:
...
What framing detection means in literature (move before the definitions):
...
The concept of similarity is is widely defined as ``\emph{the fact that people or things look or are the same}''\footnote{\url{https://dictionary.cambridge.org/dictionary/english/similarity}}. In our case, we want to focus on the specific \textbf{semantic textual similarity}, that expresses how two different pieces of text are similar considering their semantics.
This narrower definition is used in the NLP community, and there exists even a benchmark that evaluates and scores the models capacity of seeing the semantic similarity or difference between texts.\footnote{\url{http://nlpprogress.com/english/semantic\_textual\_similarity.html}}
