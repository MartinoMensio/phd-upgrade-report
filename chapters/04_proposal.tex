...
But since not all the differences are related to framing, we need a model that, given as input a specific difference, estimates whether this difference is meaningful in the framing perspective or not.
There are differences that are not conveying any framing, and therefore should be discarded.
In Figure~\ref{fig:framing_model} we can see that from a pair of details that are different but belong to similar contexts (e.g., a word that has been swapped for one other, or a detail that is present in one article and not in the other), we need a model that learns to recognise which differences are framing the reported events differently.
In order to do so, there may be multiple features, explicit (e.g., sentiment strength, proxies of emphasis, POS) or implicit (e.g., the representations coming from a Language Model) that the model can use to recognise when this difference is a framing one, and which type of framing signal it is.
The structure of this model is inspired from similar ones that are used on articles in isolation to detect propaganda techniques~\cite{da2019fine}.
The commonality is that we have an additional layer on top of the representation layer (BERT in the reference work) which is responsible to recognise when and which specific techniques are occurring.
The difference is that we are working with a dual network which has a 
...
% dataset building
To evaluate the model proposed, we will need to expand the dataset of labelled instances, where manual annotators will be given the refined task description and will annotate on document pairs the framing differences according to the schema refined with the first user study.
